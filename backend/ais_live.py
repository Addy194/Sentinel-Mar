...
logger=logging.getLogger('ais_live'); WS_URL='wss://stream.aisstream.io/v0/stream'; DEFAULT_BBOXES=[[[50.0,-5.0],[62.0,12.0]]]
...
async def _demo_cycle():
 ...
 now=datetime.now(timezone.utc).replace(second=0,microsecond=0); positions=[]; vessels=[('211100001','DEMO TANKER 01',56.1,3.2,85),('211100002','DEMO CARGO 02',55.6,4.5,110),('211100003','DEMO TUG 03',57.0,5.4,250)]
 for m,n,lat,lon,cog in vessels:
  for j in range(12): positions.append(_position(m,n,lat+0.02*j,lon+0.035*j,now-timedelta(minutes=(11-j)*10),9+j%3,cog))
...
async def _run():
 while True:
  ...
  if not cfg['enabled']: state['connected']=False; await asyncio.sleep(5); continue
  if cfg['mode']=='demo':
