...
export const DetectorFeedback = ({ caseId, source, onSaved }) => {
  ...
  const latest = rows[0];
  return (
    <div className="rounded border p-3" style={{ borderColor: "rgba(157,78,221,0.45)", background: "rgba(157,78,221,0.05)" }} data-testid="detector-feedback">
      <div className="mb-1 flex items-center gap-2"><span className="font-display text-sm font-semibold">Detector feedback</span><span className="font-mono text-[10px] text-slate-500">{source}</span>
        {latest && <span data-testid="detector-feedback-latest" className="ml-auto rounded px-1.5 py-0.5 font-mono text-[10px]" style={{ color: latest.verdict === "true_positive" ? "#10B981" : latest.verdict === "false_positive" ? "#FF2A6D" : "#FFB703", border: "1px solid currentColor" }}>{latest.verdict.replace("_", " ")}{latest.reason ? ` · ${latest.reason}` : ""} · {latest.user_email}</span>}</div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {[["true_positive", ThumbsUp, "#10B981", "True positive"], ["false_positive", ThumbsDown, "#FF2A6D", "False positive"], ["uncertain", HelpCircle, "#FFB703", "Uncertain"]].map(([v, Icon, col, l]) => (
          <button key={v} data-testid={`feedback-${v}`} onClick={() => setVerdict(v)} className="inline-flex items-center gap-1 rounded border px-2.5 py-1 font-mono text-[10px] uppercase tracking-wider" style={{ borderColor: verdict === v ? col : "var(--border-highlight)", color: verdict === v ? col : "#CBD5E1", background: verdict === v ? `${col}1a` : "transparent" }}><Icon size={11} /> {l}</button>
        ))}
      ...
      {verdict && (
        <div className="mt-2 flex gap-2">
          ...
          <button data-testid="feedback-submit" disabled={busy} onClick={submit} className="shrink-0 rounded px-3 py-1.5 font-mono text-[10px] font-semibold uppercase tracking-wider text-slate-950 disabled:opacity-50" style={{ background: "#9D4EDD" }}>Record verdict</button>
        </div>
      )}
