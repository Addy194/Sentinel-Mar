...
export const EmailSettings = ({ onChanged }) => {
  ...
  const load = () => api.get("/settings/email").then((r) => { setS(r.data); setSender(r.data.sender_email || ""); }).catch((e) => toast.error(apiError(e)));
  useEffect(() => { load(); }, []);
  ...
  return (
    <div className="panel p-5 fade-up" data-testid="email-settings">
      <div className="mb-3 flex items-center gap-2"><Mail size={16} color="#00F0FF" /><h2 className="font-display text-lg font-semibold">Email delivery (Resend)</h2>
        <span data-testid="email-configured-badge" className="ml-auto rounded-full px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-wider" style={{ color: s.configured ? "#10B981" : "#FFB703", border: `1px solid ${s.configured ? "#10B981" : "#FFB703"}66` }}>{s.configured ? "configured" : "not configured"}</span></div>
      <p className="mb-3 text-xs text-slate-400">Password-reset links are emailed through Resend. Create a key at resend.com/api-keys and verify a sender domain; <span className="font-mono">onboarding@resend.dev</span> works for test mode only.</p>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <button data-testid="btn-test-email" disabled={busy || !s.api_key} onClick={test} className="inline-flex items-center gap-1.5 rounded border px-3 py-1.5 font-mono text-[11px] uppercase tracking-wider text-slate-300 hover:text-white disabled:opacity-50" style={bd}><Send size={12} /> Send test to me</button>
      ...
      {s.last_test && <p className="mt-2 font-mono text-[10px]" style={{ color: s.last_test.sent ? "#10B981" : "#FF2A6D" }} data-testid="email-last-test">last test {fmtTime(s.last_test.at)} → {s.last_test.sent ? `sent (${s.last_test.id})` : `failed: ${s.last_test.error}`}</p>}
    </div>
  );
};
