  ...
  return (
    <div className="panel p-5 fade-up" data-testid="live-ais-panel">
      <div className="mb-2 flex items-center gap-2"><Radio size={16} color="#00F0FF" /><h2 className="font-display text-lg font-semibold">Live global AIS feed</h2>
        <span data-testid="live-ais-badge" className="ml-auto inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 font-mono text-[10px] uppercase tracking-wider" style={{ color: s.connected ? "#10B981" : s.configured ? "#FFB703" : "#94A3B8", border: `1px solid ${s.connected ? "#10B981" : s.configured ? "#FFB703" : "#94A3B8"}66` }}>{s.connected ? <Wifi size={10} /> : <WifiOff size={10} />} {s.connected ? "streaming" : !s.configured ? "not configured" : s.enabled ? "connecting…" : "paused"}</span></div>
      <p className="mb-3 text-xs text-slate-400">{s.provider} — terrestrial AIS position reports streamed into the AIS store for any region, so correlation works beyond the demo data. Coverage depends on shore-station density; ocean gaps are expected.</p>
      ...
      {admin && (
        <div className="mt-3 grid gap-2 sm:grid-cols-[1fr_180px_auto_auto]">
          ...
          <button data-testid="btn-save-live-ais" disabled={busy} onClick={() => save({ ...(key ? { api_key: key } : {}), bboxes: REGIONS[region], enabled: true })} className="rounded bg-cyan-400 px-3 py-1.5 font-mono text-[11px] font-semibold uppercase tracking-wider text-slate-950 hover:bg-cyan-300 disabled:opacity-50">Save & start</button>
          <button data-testid="btn-toggle-live-ais" disabled={busy || !s.configured} onClick={() => save({ enabled: !s.enabled })} className="rounded border px-3 py-1.5 font-mono text-[11px] uppercase tracking-wider text-slate-300 hover:text-white disabled:opacity-50" style={bd}>{s.enabled ? "Pause" : "Resume"}</button>
        </div>
      )}
    </div>
  );
