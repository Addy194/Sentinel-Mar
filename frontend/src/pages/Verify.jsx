  ...
  return (
    <div className="min-h-screen px-6 py-12" style={{ background: "var(--bg-primary, #0A0E17)", color: "#F8FAFC" }} data-testid="verify-page">
      <div className="mx-auto max-w-2xl">
        ...
        {res && (
          <div data-testid="verify-result" className="mt-6 rounded border p-4" style={{ borderColor: ok ? "#10B981" : "#FF2A6D", background: ok ? "rgba(16,185,129,0.08)" : "rgba(255,42,109,0.08)" }}>
            <div className="flex items-center gap-2 font-display text-lg font-semibold">{ok ? <ShieldCheck color="#10B981" /> : <ShieldAlert color="#FF2A6D" />}<span data-testid="verify-status">{ok ? "VERIFIED — integrity intact" : `NOT VERIFIED — ${res.status}`}</span></div>
            ...
            {res.files && <div className="mt-3 space-y-0.5 font-mono text-[10px]" data-testid="verify-files">{res.files.map((f) => <div key={f.file} className="flex gap-2"><span style={{ color: f.ok ? "#10B981" : "#FF2A6D" }}>{f.ok ? "✓" : "✗"}</span><span className="text-slate-300">{f.file}</span><span className="ml-auto text-slate-500">{f.actual?.slice(0, 16)}…</span></div>)}</div>}
          </div>
        )}
      </div>
    </div>
